# -*- coding: utf-8 -*-
"""Direct card pure protocol binding card + Payment confirmation (zero browser)。

core breakthrough (refer to zkky):
  Stripe Card tokenization pure HTTP The key is not POST /v1/payment_methods (The endpoint
  right pk direct return 400 "integration surface unsupported"), Instead, the card data is inlined into
  SetupIntent confirm In the call — This is exactly Stripe.js confirmCardSetup things done internally。

process (strong_bind_direct model):
  1. (Already) lift chain: checkout(PH/US) → update(TR press0) → oaics short chain
  ...
  4. (HTTP) POST /v1/setup_intents/{seti}/confirm — Inline card data → pm_id + succeeded
  5. (HTTP) GET  /backend-api/payments/payment_methods — Verify card binding
...
All pure HTTP, zero browser, zero CDP。
"""
...
# hCaptcha (Stripe invisible, Optional)
...
# Stripe public key (Public configuration, Dual merchant sharding)
...
def _pick_pk(client_secret: str) -> str:
    """according to SetupIntent merchant shard selection pk。"""
...
def _walk(value: Any):
    """recursion yield all dict value sum list element。"""
...
def _find_key(payload: Any, names: tuple[str, ...]) -> str:
    """Search for the first match in a nested structure key value。"""
...
def _find_identifier(payload: Any, prefixes: tuple[str, ...]) -> str:
    """Search for the first string value starting with a certain prefix in a nested structure。"""
...
def _setup_intent_id(payload: Any, client_secret: str = "") -> str:
    """from payload or client_secret extracted from setup_intent_id。"""
    # from client_secret extract
...
def _publishable_key_for_setup(client_secret: str, fallback: str = "") -> str:
    """from client_secret Merchant sharding inference pk。"""
...
def _stripe_headers(
    ...
    *,
    ...
    """Stripe API Request header (key: Origin=js.stripe.com)。"""
    h = {
...
# =============================================================================
# Step 1: create SetupIntent (ChatGPT API)
# (Keep original create_setup_intent, It can already run through)
# =============================================================================
...
# =============================================================================
# Step 2: SetupIntent confirm — Inline card data (core breakthrough!)
# replace old create_payment_method + confirm_setup_intent two steps
# =============================================================================
def setup_confirm_inline(
    ...
    *,
    ...
    """POST /v1/setup_intents/{seti}/confirm — inline payment_method_data。

    This is the core of the pure protocol: Card number/CVC/Validity period as payment_method_data[card][...] Field
    Send directly to confirm endpoint, Stripe The server is in confirm automatically created during the process PaymentMethod,
    No need to adjust first /v1/payment_methods (This endpoint pairs pk direct return 400)。
    """
    cf = _card_fields(card)
    bl = _billing_fields(billing)
    sid = stripe_js_id or str(uuid.uuid4())

    # Stripe Anti-fraud field (Normally by Stripe.js Generate in browser cookie)
    guid = f"{uuid.uuid4()}{uuid.uuid4().hex[:6]}"
    muid = f"{uuid.uuid4()}{uuid.uuid4().hex[:6]}"
    sid_val = f"{uuid.uuid4()}{uuid.uuid4().hex[:6]}"

    # from init_payload extract elements_session_id / wallet_config_id (If any)
    elements_session_id = ""
    wallet_config_id = ""
...
# =============================================================================
# Step 3: Card verification (ChatGPT API + Stripe API)
def list_payment_methods(
    ...
    """GET /backend-api/payments/payment_methods — Confirm card binding。"""
    s = chatgpt_session(proxy, access_token, session_token)
...
def list_stripe_payment_methods(
    ...
    """GET /v1/payment_methods?customer=cus_xxx — Stripe Side column cards。"""
    r = session.get(
...
# =============================================================================
# Step 4-6: Payment confirmation section (confirmation_tokens → checkout/confirm → final confirm)
# =============================================================================
def create_confirmation_token(
    ...
    """POST /v1/confirmation_tokens — use pm_id Change ctoken。"""
    body: dict[str, Any] = {
...
def checkout_confirm(
    ...
    """POST /backend-api/payments/checkout/confirm — take final SetupIntent。"""
...
def final_setup_confirm(
    ...
    """POST /v1/setup_intents/{final}/confirm — use ctoken Confirm final payment。"""
    return_url = (
...
# =============================================================================
# Step 7: Subscription verification
# =============================================================================
def verify_subscription(
    session, *, access_token: str, account_id: str,
    max_attempts: int = 3, interval_sec: float = 0.35, timeout: int = 20,
) -> dict[str, Any]:
    """polling GET /backend-api/subscriptions confirm plus activation。"""
...
# =============================================================================
# Complete pure protocol card binding (Step 1-3: bind only)
# =============================================================================
def bind_card(
    proxy: str,
    access_token: str,
    ...
    *,
    ...
    """Pure protocol card binding: SetupIntent → inline confirm → Card verification。
    ...
    Args:
        ...
        checkout_id: oaics_xxx (From the chain)
        processor: openai_llc / openai_phoenix_llc
        billing: billing address (Optional, Default card name)
        hcaptcha_token: hCaptcha passive token (Optional)
    ...
    try:
        # --- Step 1: checkout context (If any checkout_id) ---
        ...
        # --- Step 3: SetupIntent confirm — Inline card data (core!) ---
...
# =============================================================================
# Complete pure protocol payment (Step 1-7: bind + pay + verify)
# =============================================================================
def bind_and_pay(
    proxy: str,
    access_token: str,
    ...
    *,
    ...
    """Pure protocol complete process: Bind card → confirmation_token → checkout/confirm → final confirm → Subscription verification。

    Args:
        checkout_id: from the chain oaics_xxx
        processor: openai_llc / openai_phoenix_llc
        billing: billing address (Contains name/line1/city/state/postal_code/country)
        currency: USD / EUR wait
        fast_verify: True=Check only subscriptions; False=Extra check verify/success.data/auth/session
    ...
    s = chatgpt_session(proxy, access_token, session_token)
    try:
        # --- Phase 2: Payment confirmation (Step 5-6) ---
        ...
        # --- Phase 3: Subscription verification (Step 7) ---
