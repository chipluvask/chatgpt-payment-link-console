...
class PayPalFlow:
    ...
    def _emit_progress(self, step: str, detail: str = "", *, level: str = "info") -> None:
        """to the outside (API layer) Push process progress: progress_cb(step, detail, level)。

        Callback exceptions never interrupt the main process; detail Truncated too long。
        """
    ...
    def _context_country(self) -> str:
        """Authorization segment country context (sole source of truth); Fallback without context address/BR。"""
        if self.country_context is not None:
    ...
    def _cleanup_headless_session(self) -> None:
        ...
        if session is None:
            return
        # Playwright browser.close() No timeout, Can be permanently suspended when the proxy link is abnormal. (8/14 BA-3YK 380s Root cause of stuck)。
        # Put background thread + join time out: Give up cleaning after timeout, Does not block the main process ending (The authorization result has been generated)。
    ...
    def run(self) -> dict[str, object]:
        ...
        try:
            for flow_attempt in range(1, self.max_flow_attempts + 1):
                ...
                try:
                    self._emit_progress("submit_email", "Phase 0: Initialize session / DataDome Over the wall", level="info")
                    self._phase0_initial_load()
                    self._emit_progress("captcha", "Phase 2: Create an account / EC checkout check", level="info")
                    self._phase2_create_account()
                    self._emit_progress("sms", "Phase 3: Fill in the form + 2FA SMS verification", level="info")
                    self._phase3_signup_and_2fa()
                    self._emit_progress("consent_ba", "Phase 4: final authorization (billing.authorize)", level="info")
                    result = self._with_risk_runtime_report(self._phase4_authorize())
                except Exception as attempt_error:
                    self._emit_progress("submit_email", f"Process exception: {self._safe_error_text(attempt_error)[:200]}", level="err")
                    if (
                ...
                if result.get("status") == "success":
                    ...
                    self._emit_progress("done", "Authorization completed: EUAT Close up", level="ok")
                    return result
    ...
    def _frontend_disable_authchallenge_close(
        ...
        *,
        ...
        self._emit_progress(
            "captcha",
            f"authchallenge {captcha_type} Used frontend_disable closure (synthetic close)",
            level="ok",
        )
    ...
    def _extract_recaptcha_site_key(challenge_html: str) -> str:
        ...
        for pattern in (
            ...
            # PayPalObjects managed v2 page: recaptcha_v2.html?siteKey=...
    ...
    def _mint_recaptcha_v3_token_via_capsolver(
        ...
        *,
        ...
        """local Playwright reCAPTCHA solution (Replaces removed CapSolver)。

        Prioritize opening the truth challenge Page (session with protocol cookies) let widget self-explanation;
        Retreat if failed bridge (v3 execute)。return (token, solution)。
        """
        ...
        if not site_key:
            logger.warning("recaptcha site_key Failed to extract, Unable to solve locally")
            return "", {}
        cookies = []
        try:
            cookies = self.session.export_cookies_for_browser()
        ...
        if token:
            ...
            return token, {}
        logger.warning("recaptcha local solve fail (site_key={} action={})", site_key[:16], action)
        return "", {}
    ...
    def _validate_authchallenge_if_possible(self, challenge_html: str, signup_url: str):
        ...
        self.captcha_bypass_mode = paypal_captcha_bypass_mode()
        # frontend_disable Skip mode reCAPTCHA mint: pure HTTP mint of Enterprise v2 token
        # must be soft-reject, reality POST validatecaptcha Instead, the session is invalidated. (8/14 BA-2GM93699 Root cause of failure)。
        # Go directly synthetic close (8/11 path to success)。PAYPAL_RECAPTCHA_SOLVER_ENABLED=0 Explicitly close。
        if (
    ...
    def _post_authchallenge_form_close_once(
        ...
        *,
        ...
        if "hcaptchapassive" in captcha_type or self._extract_hcaptcha_passive_iframe_src(challenge_html):
            ...
            # PayPal recaptcha_v2.html Browser fill in g-recaptcha-response; Double write insurance
        ...
        for item in items:
            ...
            for err in item.get("errors") or []:
                if isinstance(err, str):
                    # GraphQL May directly return a string error (like "BUYER_NOT_SET")
    ...
    def _card_issuer_type(self) -> str:
        """PayPal GraphQL CardIssuerType enum (JCB/TROY Correct mapping, See identity_lib.issuer_type_for)。"""
        try:
    ...
    def _update_user_phone(self, phone: str):
        """Update the phone fields used by the signup/2FA GraphQL calls (Country related prefix)。"""
        raw = (phone or "").strip()
    ...
    def _context_phone_cc(self) -> str:
        """The numeric part of the mobile country code of the country context (Contextless fallback 55)。"""
        if self.country_context is not None:
    ...
    def _confirm_phone_with_retry(self, token: str, signup_url: str):
        ...
        while True:
            try:
                ...
                while True:
                    value = input(
                        "\n>>> Failed to send verification code。Please enter a new mobile phone number to resend"
                        "（like +5591980133818）；enter q quit: "
                    ).strip()
                    ...
                    try:
                        ...
                        logger.warning("Invalid mobile number：{}。Please re-enter。", phone_error)
                continue
            logger.info("SMS verification code sent to phone: {}", self._masked_phone())

            while True:
                value = input(
                    "\n>>> enter6SMS verification code；If you need to change your number，Enter your new mobile number directly"
                    "（like +5591980133818 or phone:+5591980133818）；enter q quit: "
                ).strip()
                ...
                if len(value) == 6 and value.isdigit():
                    if self._confirm_2fa_phone_confirmation(
                        ...
                        return
                    logger.warning(
                        "Verification code verification failed。You can continue to enter new6Verification code，Or enter a new mobile phone number to resend the verification code。"
                    )
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        "The input is neither6Verification code，It’s not a valid mobile phone number either.：{}。Please re-enter。",
                        e,
                    )
    ...
    def _confirm_phone_with_sms_provider(self, token: str, signup_url: str) -> None:
        ...
        # flow_id = This time flow session id; same flow internal weight running 2FA Reusable confirmed number, across flow Not reused
        ...
        for attempt in range(1, self.sms_provider.max_attempts + 1):
            try:
                ...
                # All attempts by suppliers within the range have failed. (Contains NO_NUMBERS/NO_BALANCE/cooldown):
                # After cooling down, continue to the next round of number taking. (max_attempts wheel), Don't give up directly
                ...
                self._emit_progress(
                    "sms",
                    f"Number retrieval failed (No. {attempt}/{self.sms_provider.max_attempts} wheel): {exc} — 2s Try again later",
                    level="warn",
                )
                time.sleep(2.0)
                continue
            ...
            if getattr(activation, "reused", False):
                # Reuse confirmed number (same flow rerun 2FA): no money, No price written back, Prompt reuse
                self._emit_progress(
                    "sms",
                    f"Reuse confirmed number provider={activation.provider_id} phone={self._masked_phone()} (No new deductions)",
                    level="info",
                )
            else:
                self._emit_progress(
                    "sms",
                    f"Number taken provider={activation.provider_id} price=${activation.price:.4f} phone={self._masked_phone()}",
                    level="info",
                )
            try:
                auth_id, challenge_id = self._initiate_2fa_phone_confirmation(token, signup_url)
            ...
            if self._confirm_2fa_phone_confirmation(token, signup_url, auth_id, challenge_id, code):
                ...
                self._emit_progress("sms", "OTP Confirmed successfully (CONFIRMED)", level="ok")
                return
    ...
    def _phase4_authorize(self) -> dict[str, object]:
        ...
        try:
            ...
            if not isinstance(auth_data, dict):
                ...
                # Extract the true cause of the first error (data/message/contingency), Easy to observe
                err_reason = ""
                ...
                if buyer_not_set:
                    ...
                    self._emit_progress(
                        "consent_ba",
                        f"authorize return BUYER_NOT_SET (Membership is not bound checkout), The outer layer will restart",
                        level="err",
                    )
                else:
                    logger.error(
            ...
            self._emit_progress(
                "consent_ba",
                f"authorize success: paymentAction={auth_data.get('paymentAction')} buyerUserId={self.state.user_id}",
                level="ok",
            )
            ...
            # Reconciliation layer (core.reconciliation): Verify whether the merchant side confirms successfully along the whitelist bounce chain
            reconciliation = None
            try:
