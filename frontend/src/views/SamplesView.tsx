...
function CountryCell({ s }: { s: Sample }) {
  ...
  if (act && req && act !== req) {
    return (
      <span className="tag" title={`Configuration ${req} → actual ${act}${s.exit_ip ? `, exit ${s.exit_ip}` : ""}`}>
        {req}→{act}⚠
      </span>
    );
  }
  if (act) {
    return (
      <span className="tag" title={s.exit_ip ? `exit ${s.exit_ip}${s.geo_confidence ? `, Confidence ${Math.round((s.geo_confidence || 0) * 100)}%` : ""}` : undefined}>
        {act}
      </span>
    );
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Fall back to when network abnormality occurs mock
      }
  ...
  return (
    <div className="page">
      <div className="page-head">
        <div>
          <h2 className="page-title">sample record</h2>
          <p className="page-sub">View details of successful and failed samples</p>
        </div>
