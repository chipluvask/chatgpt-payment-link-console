...
def _connect_roxy_browser(playwright: _Playwright, roxy_browser: JsonObject, *, timeout_ms: int = 30000) -> _Browser:
    ...
    if not isinstance(cdp_info, dict) or not (cdp_info.get("ws") or cdp_info.get("http")):
        raise LocalHeadlessRuntimeError("Roxy browser CDP Information does not exist，Unable to connect")
    try:
        from paypal.roxy_fingerprint import _connect_over_cdp

        endpoint = _connect_over_cdp(cast(dict[str, Any], cdp_info))
    except Exception as exc:
        raise LocalHeadlessRuntimeError(f"Roxy browser CDP endpoint invalid: {exc}") from exc
...
class LocalHeadlessSession:
            ...
            try:
                if run_mtr and script_url == dfp_script_url:
                    ...
                    try:
                        ...
                        # add_script_tag Go within the page fetch, subject page CSP Throttling may fail;
                        # rollback: After downloading content with an agent evaluate Direct execution (bypass CSP fetch intercept)
    ...
    def _fetch_inject_script(self, script_url: str) -> str:
        """Download script content with proxy, for evaluate Inject fallback (bypass add_script_tag of CSP fetch limit)。"""
        try:
            if self.proxy_url:
