"""PayPal Merchant bounce chain reconciliation layer (from GPT-Register-Tool paypal_reconciliation.py transplant)。

core: reconcile_paypal_return Jump back along the whitelist chain
  pm-redirects.stripe.com -> pay.openai.com -> chatgpt.com(/checkout/verify)
Hop-by-hop verification (URL structure + Stage jump legality + HTTP state + body tag),
output secret-free of conclusive / unknown / failed evidence。

use: lift chain/Authorized BA/pay URL back, verify"Is confirmation successful on the merchant side?",
unknown(requires_reconciliation) Never automatically retry (avoid duplication side-effect)。
"""
...
class ReturnURLValidationError(ValueError):
    """safe checksum error, message never contains rejected URL。"""
...
def normalize_return_state(url: str) -> NormalizedReturnState:
    """Verify a bounce URL, Only exposed to the normalized state after desensitization。"""
...
def reconcile_paypal_return(
    ...
    *,
    ...
    """Verify and classify hop-by-hop along the whitelist bounce chain。

    transport Do not automatically follow the jump (This function always passes allow_redirects=False, Verify before requesting each hop)。
    """
...
class CurlTransport:
    """This project transport: Package curl_cffi session (chain.make_session / PayPalSession)。"""
