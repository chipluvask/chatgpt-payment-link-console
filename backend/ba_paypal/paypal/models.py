...
class UserInfo:
    ...
    # Nationalized extension fields (default None/null, backwards compatible)
...
def generate_card(
    ...
    """Nationalized card generation: by country BIN Pool selection number (Fallback not included US), Luhn Check digit, CVV Length varies issuer。

    old signature `generate_card(proxy_url=...)` reserve (default country="BR")。
    """
    del proxy_url
...
def generate_user(phone: str = "", country: str = "BR") -> UserInfo:
    """Nationalized identity generation。

    country != BR: entrust identity_lib (Name/Mail/Birthday/Documents/address/Telephone nationalization)。
    country == BR: Keep original Brazilian path (backwards compatible)。
    """
...
def _normalize_phone_full(phone: str, default_cc: str) -> str:
    """Unify mobile phone numbers in any format +CC Beginning with complete number。

    null value: Generate synthetic national number (After the code is activated, it will be _update_user_phone replace, Not true);
    Explicit illegal value: throw error。
    """
...
def _synthetic_phone(cc_prefix: str) -> str:
    """synthesize one"local structure real but not real number"place number (Subsequently, it will be covered by the receiving number.)。"""
    try:
...
def generate_address(country: str = "BR") -> BillingAddress:
    """Nationalized billing address。country != BR entrust identity_lib address pool (line2 Semantics by country)。"""
    cc = (country or "BR").strip().upper()
    if cc != "BR":
        ...
        # line1 Embedded house number (like "Av Paulista 1000") Time and minute travel number field
        house_number = ""
        street = line1
