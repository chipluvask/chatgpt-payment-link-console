...
/** segment label: If there is a real country, it will show the real + drift/Reuse tag */
...
function ChainCardInner({ chainId, cs, onClick }: Props) {
  ...
  const stages = order.map((stage) => {
    ...
    const driftTip = sd?.drifted
      ? ` (Configuration ${sd.country} → actual ${sd.actualCountry})`
      : sd?.reusedFrom
        ? ` ⇄ Reuse ${sd.reusedFrom} section exit IP`
        : sd?.exitIp
          ? ` · exit ${sd.exitIp}${sd.geoConfidence ? ` · Confidence ${Math.round((sd.geoConfidence || 0) * 100)}%` : ""}`
          : "";
    return (
  ...
  let current: { text: string; cls: string } = { text: "Waiting to begin", cls: "badge-muted" };
  ...
  if (cs.status === "success") current = { text: isOaics ? "✓ OAICS Successfully lifted the chain" : "✓ Successfully lifted the chain", cls: "badge-success" };
  else if (cs.status === "failed") current = { text: `✗ ${cs.reasonText || cs.reason || "fail"}`, cls: "badge-danger" };
