...
def paypal_captcha_bypass_mode() -> str:
    ...
    if not normalized:
        # default frontend_disable: authchallenge Use local synthetic close bypass
        # (8/11 BA-XXXXXXXXXXXXXXX2 path to success), Does not rely on explicit configuration of environment variables。
...
class PayPalSession:
    ...
    def __init__(
        ...
        self.proxy_label = proxy_label or ("Agent is enabled" if proxy_url else "Agent closed")
        ...
        client_kwargs: dict[str, Any] = {
            ...
            # ensure“Turn off proxy”Not always HTTP_PROXY/HTTPS_PROXY Environment variables unexpectedly taken over。
            "trust_env": False,
        }
        if proxy_url:
    ...
    def _request_with_retry(self, method: str, url: str, kwargs: dict[str, Any], req_id: Any):
        """Network layer retry: Proxy tunnel only/Connection class transient error retry (502 CONNECT / time out / Disconnect)。

        711 Waiting for residential agent to happen occasionally CONNECT 502, Try again 2 can be crossed; HTTP 4xx/5xx Response is not retried。
        """
