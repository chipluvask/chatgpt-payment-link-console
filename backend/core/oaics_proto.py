# -*- coding: utf-8 -*-
"""oaics_ custom Checkout pure HTTP Protocol layer (Alignment link-pp handoff/protocol/stripe_checkout.py)。

checkout(custom) -> fetch state -> taxes -> strict 0 meta polling -> elements/sessions
(amount=0 + manual approval betas) -> confirmation_tokens (inline paypal, Optional P1)
-> checkout/confirm (Optional sentinel head) -> Straight out redirect, otherwise Stripe intent
confirm (seti_/pi_, Reuse ctoken) -> redirect。not pass cs_live_ of update/approve/poll。
"""
...
# OpenAI Web Front-end version header (Alignment link-pp chatgpt_context_headers)
_CHATGPT_CLIENT_VERSION = "prod-db390ebea64862bf1899c420a4c736e0cf639747"
_CHATGPT_CLIENT_BUILD_NUMBER = "7904904"

# Stripe.js Browser TLS/client head (Alignment link-pp _stripe_headers)
...
def _stripe_headers() -> dict[str, str]:
    """Alignment link-pp _stripe_headers: browser context + UA + sec-ch-ua family。"""
    return {
...
def _cookie_header_for(device_id: str, session_token: str) -> str:
    """Structure and chatgpt_session consistent oai-did + session token Cookie head。

    refer to link-pp _session_cookie_header: sentinel /req Must carry and follow up
    ChatGPT Requesting the same session cookie。
    """
...
def _merge_set_cookie(cookie_jar: Optional[dict], resp) -> None:
    """respond Set-Cookie Merge into chain level cookie Can (Only first name reserved=value, Ignore expiration/Domain attributes)。

    Alignment link-pp CheckoutTransport.claim session retention: Issued by the server
    __cf_bm / CSRF kind cookie Need to be transparently passed to subsequent requests, otherwise confirm/sentinel rejected。
    """
...
def _cookie_header(device_id: str, session_token: str,
    ...
    """Assemble Cookie head: Base oai-did/session token + Server issued cookie pass-through。"""
...
# Every device stable oai-session-id (Alignment link-pp oai_session_id_for_device)
_OAI_SESSION_IDS: dict[str, str] = {}
_OAI_SESSION_IDS_LOCK = threading.Lock()

# Browser files by country (locale/timezone/language), Fallback for countries not included en-US
_LOCALE_PROFILES = {
...
class OaicsPromoNotApplied(RuntimeError):
    def __init__(self, session_id: str, amount: Any, currency: str, detail: str = ""):
        ...
        super().__init__(
            "The free promotion does not actually take effect "
...
class OaicsPaypalUnsupported(RuntimeError):
    def __init__(self, session_id: str, methods: list[str], detail: str = ""):
        ...
        suffix = f"; {detail}" if detail else ""
        super().__init__(
            f"Checkout Not supported PayPal (session={session_id}, pm={self.methods}){suffix}"
        )
...
class PayPalRiskDeclined(RuntimeError):
    """Stripe Radar Rejected current PayPal Payment method (Alignment link-pp PayPalRiskDeclinedError)。"""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            f"PayPal Risk control rejection: decline_code={self.decline_code}{detail}"
        )


class PayPalFundingUnavailable(RuntimeError):
    """current Checkout Not accepted PayPal (Alignment link-pp PayPalFundingUnavailableError)。"""
    ...
    def __init__(self, session_id: str, payment_method_types: list[str], detail: str = ""):
        ...
        suffix = f"; {detail}" if detail else ""
        super().__init__(
            f"Checkout Not supported PayPal "
            f"(session={self.session_id}, pm={self.payment_method_types}){suffix}"
        )


def _is_payment_method_types_mismatch(resp) -> bool:
    """confirm return payment_method_types_mismatch Determine the current time Checkout Not supported PayPal。"""
    try:
        payload = resp.json()
    except Exception:
        payload = {}
...
def _raise_for_current_paypal_risk_decline(payload: Any, payment_method_id: str = "") -> None:
    """only if generic_decline Belongs to this time provider used PM thrown when (Alignment link-pp)。"""
...
def common_headers(*, country: str, device_id: str, referer: str, route: str = "") -> dict[str, str]:
    ...
    headers = {
        ...
        # 2026-08-14: Alignment link-pp chatgpt_context_headers repair sec-ch-ua family + sec-fetch
        "sec-ch-ua": _STRIPE_SEC_CH_UA,
        "sec-ch-ua-full-version-list": _STRIPE_SEC_CH_UA_FULL,
        "sec-ch-ua-mobile": "?0",
        "sec-ch-ua-platform": '"Windows"',
...
def warmup_chatgpt_page(session, *, country: str, device_id: str, timeout: float = 30) -> None:
    """Same session GET chatgpt.com Warm up page, kind __cf_bm/_cfuvid cookie (Alignment link-pp)。

    Call before placing an order, let CF/OAI cookie and follow-up Checkout POST same HTTP session;
    Failure is silently ignored (Does not affect order establishment)。
    """
    try:
...
def oai_session_id_for_device(device_id: str) -> str:
    """Return with device Binding stability session UUID (Alignment link-pp)。"""
...
def bootstrap_oaics_checkout_context(
    ...
    """preheat checkout Page capture front-end deployment context (Alignment kakao _bootstrap_oaics_kakao_http_context)。

    return webDeploymentAttestation / sessionId / data-build / data-seq,
    confirm use oai-web-deployment-attestation + x-oai-is-client-observation bring out。
    MIN_OAICS_ATTESTATION Can be injected directly (Skip crawling); Any failure returns null dict Downgrade。
    """
...
def fetch_oaics_checkout_state(
    ...
    if not str(session_id or "").startswith("oaics_"):
        raise OaicsPaypalUnsupported(session_id, [], "No oaics_ session")
    checkout_url = f"https://chatgpt.com/checkout/{processor_entity}/{session_id}"
    route = f"/backend-api/payments/checkout/{processor_entity}/{session_id}"
    s = chatgpt_session(proxy, access_token, session_token, device_id=device_id)
    if cookie_jar:
    ...
    try:
        ...
        if r.status_code == 401:
            raise OaicsAuthError("read OAICS Checkout HTTP 401")
        if r.status_code != 200:
            raise RuntimeError(f"read OAICS Checkout HTTP {r.status_code}: {(r.text or '')[:300]}")
...
def submit_oaics_checkout_taxes(
    ...
    try:
        ...
        _merge_set_cookie(cookie_jar, r)
        if r.status_code == 401:
            raise OaicsAuthError("submit OAICS bill HTTP 401")
        if r.status_code != 200:
            raise RuntimeError(f"submit OAICS bill HTTP {r.status_code}: {(r.text or '')[:300]}")
        d = r.json() if r.text else {}
        return d if isinstance(d, dict) else {"raw": (r.text or "")[:400]}
    finally:
        s.close()
...
def _minor_amount(value: Any) -> Optional[int]:
    ...
    # Only accepts integer literals (Decimal points allowed are .0), reject 1.5 etc. will be truncated into integers causing
    # 0 Values ​​misjudged by meta-validation (Alignment link-pp _minor_amount)
...
def verify_oaics_zero_snapshot(payload: Any, *, session_id: str, currency: str) -> int:
    ...
    if not observations:
        raise OaicsPromoNotApplied(
            ...
            "OAICS Verifiable amount due not returned",
        )
...
def wait_for_oaics_zero(
    ...
    if last_error is not None:
        raise last_error
    raise OaicsPromoNotApplied(session_id, "unknown", currency, "stateless")
...
def create_oaics_elements_session(
    ...
    if not publishable_key.startswith("pk_live_"):
        raise RuntimeError("OAICS Lack Stripe publishable_key")
    if not customer_secret:
        raise RuntimeError("OAICS Lack customer_session_client_secret")
    ...
    try:
        ...
        if r.status_code != 200:
            raise RuntimeError(f"OAICS Elements Session fail HTTP {r.status_code}")
        d = r.json() if r.text else {}
...
def create_oaics_paypal_confirmation_token(
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            raise RuntimeError(f"OAICS ConfirmationToken fail HTTP {r.status_code}{suffix}")
        d = r.json() if r.text else {}
        confirmation_token = _oaics_find_string(
            d if isinstance(d, dict) else {},
            ("id", "confirmation_token", "confirmationToken"), prefixes=("ctoken_", "ct_"))
        if not confirmation_token:
            raise RuntimeError("OAICS ConfirmationToken Response missing token")
...
def confirm_oaics_paypal_intent(
    ...
    if "_secret_" not in intent_id_raw:
        raise RuntimeError("OAICS confirm Not returned Intent client_secret")
    ...
    if intent_id.startswith("pi_"):
        ...
        raise RuntimeError(f"OAICS confirm Return unknown Intent: {intent_id}")
    if intent_type and intent_type != expected_type:
        raise RuntimeError(f"OAICS confirm returned Intent Inconsistent types: {intent_type} != {expected_type}")
    body = {
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            # 2026-08-15: Comes with complete error JSON (param/decline_code/message) Easy to locate
            # setup_intent_invalid_parameter wait 400 The specific mismatch parameters of
            import json as _json
            ...
            if _is_payment_method_types_mismatch(r):
                raise PayPalFundingUnavailable(
                    intent_id, [], "intent confirm return payment_method_types_mismatch")
            _raise_for_current_paypal_risk_decline(d, "")
            suffix = f" ({code})" if code else ""
            raise RuntimeError(f"OAICS Intent confirm fail HTTP {r.status_code}{suffix}: {_dbg}")
        d = r.json() if r.text else {}
        _raise_for_current_paypal_risk_decline(d, "")
        return d if isinstance(d, dict) else {"raw": (r.text or "")[:400]}
    finally:
...
def oaics_standard_paypal_redirect(
    ...
    if not redirect_url:
        raise RuntimeError("OAICS PayPal confirm No jump address returned")
    return redirect_url, {
        "elements": elements,
...
def oaics_to_paypal_redirect(
    ...
    for attempt in range(1, 4):
        ...
        if payment_method_types:
            raise PayPalFundingUnavailable(
                session_id, payment_method_types, "OAICS payment_method_types Not included paypal")
        if attempt < 3:
            time.sleep(0.8 * attempt)
    if not methods:
        raise RuntimeError("OAICS Decideable not returned payment_method_types or cpmt_ Payment method")
    raise PayPalFundingUnavailable(
        session_id, [str(m.get("id") or "") for m in methods],
        "Custom payment methods are not included PayPal",
    )
...
def start_oaics_custom_payment_method(
    ...
    """Get started with custom payment methods (Alignment link-pp start_oaics_custom_payment_method)。

    POST /backend-api/payments/checkout/custom_payment_method
    → Return updated status (Contains cpmt_ Entrance / Confirm required fields)。
    """
    from .chain import chatgpt_session, _req
...
def confirm_oaics_custom_payment_method(
    ...
    """Customize payment method to continue/confirm (Alignment link-pp confirm_oaics_custom_payment_method)。

    POST /backend-api/payments/checkout/custom_payment_method/continue
    → Straight out redirect; blocked/declined Classification throws exception。
    """
    from .chain import chatgpt_session, _req
...
def oaics_custom_paypal_redirect(
    ...
    """cpmt_ Custom payment method (paypal) lift chain: start -> continue -> redirect。

    only if payment_method_types not directly exposed paypal but exists cpmt_ method used;
    start/continue Any failure will result in downgrade. PayPalFundingUnavailable (Classified by caller)。
    """
    state: dict[str, Any] = {}
    for attempt in range(1, 3):
        ...
        if "paypal" in methods:
            raise PayPalFundingUnavailable(session_id, methods, "The payment method has been directly exposed, Shouldn't go cpmt")
        cpmt = [
        ...
        if not cpmt:
            raise PayPalFundingUnavailable(
                session_id, [], "cpmt_ Custom payment methods are not included PayPal")
        ...
        if attempt < 2:
            time.sleep(0.8)
    raise RuntimeError("OAICS cpmt continue No jump address returned")