# -*- coding: utf-8 -*-
"""reg/repo_accounts.py — ChatGPT Register account warehousing（synchronous sqlite3，same tokens.db）

reg_accounts surface：Register full output records（With password/Source email/channel/error code）。
Successful account synchronization is written into this project tokens surface（source='register'），Reuse token_store
field convention，Can be used directly by the lift link。

Notice：token_store（aiosqlite）with here（synchronous sqlite3）share the same db document，
SQLite WAL Mode supports concurrent reading and writing；For writing INSERT OR IGNORE Avoid competition with the main library。
"""
...
def upsert_account(conn, rec: dict) -> int:
    """according to email idempotent write：If it already exists, update the credentials and status.，Otherwise insert。Return record id。"""
...
def push_to_tokens(conn, rec: dict) -> bool:
    """Successful registration and account synchronization writing tokens surface（source='register'）。

    Remove duplicates：access_token repeat or same email If it already exists, skip it（Alignment token_store.import_from_pool）。
    """
