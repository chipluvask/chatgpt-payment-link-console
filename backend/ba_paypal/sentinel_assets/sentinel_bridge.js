#!/usr/bin/env node
/**
 * sentinel_bridge.js —— Use it real·sdk.js（Node/V8 run）generate create_account needed
 * openai-sentinel-token（real t）+ openai-sentinel-so-token（real so Device fingerprint）。
 *
 * pure Python/httpx Can't get it so-token（it is confused by sdk.js Collect browser sensors and calculate），
 * here at Node Set up your browser shim（sentinel_bootstrap.js）+ Load true sdk.js，
 * tune SentinelSDK.__proto2(flow) take {t, so, c, seed, diff, powReq}，
 * Node side solution PoW Assemble two header value，JSON hit stdout。
 *
 * usage：Bundle {ua, cores, deviceId, flow, proxy, version} by JSON from stdin incoming。
 */
"use strict";
...
// ---- PoW（Alignment sentinel.go：configArray(answer=true) + fnv1a32 + b64） ----
...
function makeFetch(ua, language, cookieHeader, dispatcher, fetchImpl, sentinelOrigin, frameUrl) {
  // sdk inside fetch bridge to fetchImpl：rewrite URL arrive sentinel.openai.com、Make up the head、Go through an agent。
  // key：dispatcher and fetchImpl must be from the same undici Version，otherwise Node built-in fetch
  // Check external dispatcher of Request handler Report "invalid onRequestStart method"。
...
async function main() {
  ...
  // fetch + ProxyAgent Must have the same origin（same undici Version）。priority npm undici；
  // no agent and undici Return if missing Node Built-in global fetch。
  let undici = null;
  try {
    undici = require("undici");
  } catch (_) {}

  // acting（undici ProxyAgent）——Same as registration IP hit /sentinel/req。
  // Notice：undici will not be automatically parsed URL inside user:pass@，must be given explicitly Proxy-Authorization token。
  let dispatcher = null;
  ...
  if (proxy) {
    if (undici && /^https?:/i.test(proxy)) {
      try {
        ...
        const opts = { uri: pu.origin }; // http://host:port（remove userinfo）
        ...
        process.stdout.write(JSON.stringify({ error: "ProxyAgent Construction failed：" + ((e && e.message) || e) }));
        process.exit(1);
        return;
      }
    } else {
  ...
  // Node of crypto/navigator/performance/self is read-only global，Will make peace bootstrap assignment conflict。
  // Use independent vm sandbox（brand new global），only inject sdk actually needed host ability。
  const sandbox = {
    console,
    setTimeout,
    clearTimeout,
  ...
  if (!SDK || typeof SDK.__proto2 !== "function") {
    process.stdout.write(JSON.stringify({ error: "sdk.__proto2 undefined" }));
    process.exit(1);
    return;
  }
  ...
  if (!rd.c) {
    process.stdout.write(JSON.stringify({ error: "driver none c", soErr: rd.soErr || "", raw: String(rawDriver).slice(0, 300) }));
    process.exit(1);
    return;
  }
  ...
  // token Finished，graceful closing undici Log out naturally after connecting，avoid Windows libuv exit assertion
  // （This assertion occurs in stdout after writing，does not affect the results；But will let the child process exit code get dirty）。
  ...
  // reveal all the details：If there are still ref Handle stuck in event loop，2s hard exit after（unref Exit normally without blocking）。
