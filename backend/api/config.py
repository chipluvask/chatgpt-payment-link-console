"""Configure routing with bill templates。

REST:
- GET  /api/config             - Return to full running configuration
- GET  /api/billing/templates  - Return to all bill templates
- GET  /api/billing/countries  - Return to list of supported countries
- POST /api/config/stage       - Update single segment configuration (Front-end editable)
"""
...
async def get_config():
    """Return to full running configuration（Read-only display of front-end settings page + Editable segment configuration）。"""
    raw = settings.raw
    ...
    return {
        ...
        "momo": {
            ...
            "patches": [
                {"name": "connect_intercept", "desc": "L1: intercept api.stripe.com CONNECT",
                 "enabled": settings.momo_cfg.get("connect_intercept", True)},
                {"name": "dns_fix", "desc": "L2: Clash fake-ip DoH Reparse",
                 "enabled": settings.momo_cfg.get("dns_fix", True)},
                {"name": "pm_inject", "desc": "L3: payment_method injection",
                 "enabled": settings.momo_cfg.get("pm_inject", True)},
                {"name": "confirm_build", "desc": "L4: confirm payload structure",
                 "enabled": settings.momo_cfg.get("confirm_build", True)},
                {"name": "resolve_regex", "desc": "L5: MoMo pay URL regular",
...
async def get_billing_templates():
    """Return to the list of all bill templates。"""
    templates = []
...
async def get_billing_countries():
    """Return to list of supported countries（exclude blacklist）。"""
    countries = []
...
async def update_stage_config(body: dict):
    """Update single segment configuration（write config.yaml）。"""
    stage_name = body.get("stage")
    if not stage_name or stage_name not in settings.stage_names:
        return {"ok": False, "error": f"Invalid section name: {stage_name}"}
...
async def update_branch_config(body: dict):
    """Update the overall configuration of the chain branch（switch + Seven sections），write config.yaml。"""
    ...
    if not branch_name or branch_name not in BRANCH_NAMES:
        return {"ok": False, "error": f"Invalid branch: {branch_name}, Optional {BRANCH_NAMES}"}
    ...
    # oaics Five paragraph configuration
    # 2026-08-13 abandoned: The link has been changed to follow the seven-segment mapping (oaics no longer participate in decision-making), Reserved writes are only
    # old frontend/API Echo compatible; New front end OAICS The card is displayed read-only, This branch will not be called。
    if "oaics" in body:
