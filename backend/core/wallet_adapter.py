# -*- coding: utf-8 -*-
"""Unified orchestration core of wallet channels (from GPT-Register-Tool wallet_provider.py + gcash_provider.py transplant)。

none HTTP Session ownership: caller injection WalletProviderTransport implement wire call。
Support three modes:
  - stripe_pm:    ordinary Stripe PM (gopay/grabpay) — PM/confirm/poll Walk Stripe
  - custom_pm:    custom payment method (gcash, cpmt_*, Adyen Jump)
  - midtrans_qr:  midtrans snap charge (qris) — follow After execution charge Pick QR

Output and ProtocolResult Contract field alignment (ok/status/url/link_type/retryable/error_stage/
...
# gcash custom payment method type id (Merchant side cpmt)
...
class WalletProviderTransport(Protocol):
    """wire boundary: Depend on wallet_transport.py (chain.py stack) accomplish。"""
...
def capability_result(spec: WalletMethodSpec, init_payload: Any) -> dict[str, Any]:
    """init Ability determination: Is the channel in payment_method_types/custom_payment_methods middle。"""
...
def run_wallet_provider(
    ...
    *,
    ...
    """probe Or perform the wallet jump process (checkout -> init -> PM/custom -> confirm -> approve -> poll -> follow/charge)。"""
    stage = "validation"
    spec = None
    capability = None
    ...
    try:
        ...
        # S3 payment method (stripe_pm) or custom PM (gcash)
