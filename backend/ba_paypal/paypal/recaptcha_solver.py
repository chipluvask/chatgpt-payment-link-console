# -*- coding: utf-8 -*-
"""PayPal authchallenge reCAPTCHA local solution (Playwright)。

Ported from GPT-Register-Tool captcha_solver.py, two paths:
  1. real-page: Playwright session with protocol cookies open reality challenge Page,
     recaptcha v2/v3 widget Automatic trigger (execute / point checkbox), Pick token;
     At the same time, feed new information into the browser cookie。
  2. bridge: data URI Embedded api.js render + grecaptcha.execute (v3 invisible)。
"""
...
def extract_recaptcha_context(challenge_html: str) -> dict[str, Any]:
    """from PayPal authchallenge HTML extract recaptcha context。

    return {site_key, anchor_url, enterprise, v2_checkbox}。
    anchor_url priority Enterprise anchor (PayPal use Enterprise common), otherwise api2。
    """
    ...
    # PayPal authchallenge: _adsRecaptchaSiteKey Hidden fields
    if not out["site_key"]:
...
def _enterprise_anchor_url(site_key: str, hl: str = "pt_BR") -> str:
    """PayPal Challenge the measured available parameters: co=paypalobjects.com + invisible + submit。

    2026-08-14 Actual measurement: The combination anchor carry directly recaptcha-token,
    reload out 17xx Bit Enterprise token (pure HTTP, No clicks required)。
    """
...
def solve_recaptcha_from_html(
    *,
    ...
    """unified entrance: pure HTTP enterprise anchor->reload (freecaptcha transplant) priority, Playwright bridge reveal all the details。"""
    ...
    if anchor and "size=invisible" not in anchor:
        # detect evidence: PayPal challenge use invisible+paypalobjects Parameters come out token
        anchor = ""
...
def _pw_launch_kwargs(proxy: str = "") -> dict[str, Any]:
    """Playwright launch parameter: Prioritize system use Chrome (No download required), Otherwise default bundled。"""
    kwargs: dict[str, Any] = {
        "headless": True,
...
def solve_recaptcha_bridge(
    *,
    ...
    """v3 invisible bridge solution: return token or empty string。"""
    try:
...
def solve_recaptcha_on_page(
    *,
    ...
    """open reality challenge Page, trigger recaptcha v2/v3 widget, return token。

    Try automatic first execute (v3); If the page is v2 checkbox, try to click iframe Inside checkbox。
    generated within the browser cookies will also be returned(through import Feedback from the caller)。
    """
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
    ...
    try:
        ...
        with shared_playwright() as playwright:
            ...
            try:
                ...
                # path1: v3 execute (There are grecaptcha.ready Available)
                try:
                ...
                # path2: v2 checkbox iframe Click
...
def solve_recaptcha_v3_anchor_reload(
    *,
    ...
    """pure HTTP reCAPTCHA v3 solution (transplant freecaptcha: anchor -> recaptcha-token -> reload)。

    only for standard v3 anchor (https://www.google.com/recaptcha/api2/anchor?k=...)。
    Enterprise available /recaptcha/enterprise/anchor and /recaptcha/enterprise/reload。
    """
    if not anchor_url:
        return ""
    try:
...
def solve_recaptcha(
    *,
    ...
    """unified entrance: have page_url Go to the real page, Otherwise go bridge。"""
    if page_url:
