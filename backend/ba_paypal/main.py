...
def _build_cc_sms_id(country: str) -> str:
    """National context access code (for CLI smsbower provider Select number by country)。"""
    try:
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="SMSBower Ladder low-end price ceiling (USD), default 0.05 (and API The paths are consistent)",
    )
    parser.add_argument(
        "--sms-price-high",
        type=float,
        default=None,
        help="SMSBower Ladder high-end price ceiling (USD), default 0.3 (and API The paths are consistent)",
    )
    parser.add_argument(
