# -*- coding: utf-8 -*-
"""CDP Browser card binding service (Stripe Elements tokenization the only possible channel)。

background (2026-08-01 Research conclusion):
  - Stripe Card tokenization pure HTTP hard restricted: /v1/tokens /v1/payment_methods + pk
    All returned 400 "integration surface unsupported" (Should account prohibit pk direct connection tokenization)
  - Must go Elements iframe (fingerprinted JS + postMessage protocol + Real browser fingerprint)
  - No headless Anti-detection Chrome (.ba_chrome_profile) Down hCaptcha Pass automatically
  - happy-dom wait DOM The bridge will be Stripe Fingerprint detection and identification, Not feasible

process:
  1. (HTTP) POST /backend-api/payments/payment_method -> SetupIntent client_secret
  2. (CDP) load Stripe.js + Card Element, Enter card number using real keyboard/Validity period/cvc
  3. (CDP) confirmCardSetup -> hCaptcha Pass automatically -> SetupIntent succeeded
  4. (HTTP) GET /backend-api/payments/payment_methods Verify card binding
"""
...
CHROME_PROFILE = os.environ.get(
    "MIN_CHROME_PROFILE", r"D:\tidy\min-implant\.ba_chrome_profile")
...
class CdpClient:
    """smallest CDP client (websockets + pending future)。"""
        ...
        try:
            while self._connected:
                ...
                if msg.get("id") in self.pending:
                    ...
                    # event message: distributed to on_event callback (Network.requestWillBeSent wait)
...
def ensure_chrome() -> str:
    """make sure CDP Chrome running, return ws url。"""
    ...
    try:
        ...
        pass
    # start up Chrome (No headless + Anti-detection profile)
    subprocess.Popen([
    ...
    for _ in range(15):
        ...
        try:
            ...
            continue
    raise CdpError("Unable to start CDP Chrome")


async def _type_card(cdp: CdpClient, card_sid: str, card: dict[str, Any]) -> None:
    """Real keyboard input card information to Elements iframe。"""
...
async def _find_card_iframe(cdp: CdpClient, main_sid: str) -> str:
    """try to find Card Element iframe (OOPIF target), return its sessionId。"""
    for _ in range(20):
    ...
    raise CdpError("not found Card Element iframe")


async def _apply_us_fingerprint(cdp: CdpClient, sid: str) -> None:
    """Fingerprint Environment Aligns U.S. Duty Free Billing Geography (time zone/locale/language)。

    User requirements: The fingerprint environment must be close to the geographical location of the bill filled in。Use US state for tax-free address (DE wait),
    So the fingerprints are aligned US: time zone America/New_York, locale en-US, language en-US。
    """
    try:
        # time zone + locale + language (Emulation.setTimezoneOverride / setLocaleOverride)
        ...
        # page navigator level (userAgent reserve Chrome real fingerprint, Don't change)
        log.info("CDP Fingerprints Aligned to United States (America/New_York / en-US)")
    except Exception as e:
        log.debug("CDP Fingerprint settings soft-fail: %s", e)
...
async def cdp_bind_card(
    ...
    """CDP Bind card: SetupIntent(HTTP) -> Elements Fill in the card(CDP) -> confirm(CDP) -> verify(HTTP)。"""
    ...
    try:
        ...
        # Fingerprint Environment Aligns U.S. Duty Free Billing Geography (time zone/locale)
...
async def cdp_bind_from_store(
    ...
    """Get a card from the card library and CDP Bind card。"""
    card = card_store.get_card(card_id) if card_id else card_store.pickup_card()
    if not card:
        return {"ok": False, "step": "card", "error": "No card available in card library"}
