...
def normalize_phone(value: object, cc: str = "+55") -> str:
    """Normalize and backfill by country code +CC prefix (cc like "+66" / "+1")。"""
...
class SMSBowerActivationStore:
    ...
    def reusable_activation(self, now: float | None = None, flow_id: str = "") -> SMSBowerActivation | None:
        ...
        for row in activations:
            ...
            # across flow Not reused by default (Number is a one-time resource); same flow Internal reruns can be reused, Except for blacklisting
    ...
    def record_failure(self, provider_id: str, reason: str = "") -> None:
        """Failed hierarchical accounting (2026-08-14): cooling alternative"3 permanently skipped"。

        - NO_NUMBERS  : Platform without account, short cooldown 60s (Inventory fluctuations)
        - NO_BALANCE  : Insufficient balance, Global stop buying 300s (No point in retrying)
        - Current limiting/OTPreject  : cool down 300s (The number segment is PayPal mark)
        - other        : cool down 120s
        """
...
class SMSBowerOtpProvider:
    ...
    def reserve_number(self, flow_id: str = "") -> SMSBowerActivation:
        # same flow The confirmed number can be reused within (rerun 2FA); across flow Not reused (Number is a one-time resource)
    ...
    def _purchase_new_number(self, flow_id: str = "") -> SMSBowerActivation:
        ...
        # single price range [min_price, max_price] filter (max_price for None = No upper limit)
        candidates = [
            p for p in prices
            if p.count > 0
        ...
        # Strictly try one by one in ascending order according to the actual price of the platform (Not random), After all failures, the caller will cool down and try again.
    ...
    def abandon(self, activation: SMSBowerActivation, reason: str) -> None:
        ...
        self.store.abandon(activation.activation_id)
        # No. quilt PayPal reject/time out -> Permanently blocked + provider Staged cooling
...
# ---- Quotation inquiry (read only, TTL cache; Only one call will be made to the same country in batches smsbower) ----
...
def build_provider_for_quote(country: str, service: str | None = None) -> list[dict[str, object]]:
    """Returns a list of the lowest priced suppliers available in a country (quote API use, Read only inactive)。"""
