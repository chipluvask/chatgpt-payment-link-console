    ...
    def parse(cls, raw: str) -> "ProxyEntry":
        ...
        if "://" in value:
            ...
            if parsed.scheme not in {"http", "https", "socks5", "socks5h"}:
                raise ValueError(f"Unsupported proxy protocol：{parsed.scheme}")
            if not parsed.hostname or not parsed.port:
                raise ValueError("acting URL must contain host and port")
            return cls(
        ...
        parts = value.split(":", 3)
        if len(parts) != 4:
            raise ValueError("The proxy format should be host:port:username:password")
        host, port_text, username, password = [part.strip() for part in parts]
        if not host:
            raise ValueError("acting host cannot be empty")
        try:
            port = int(port_text)
        except ValueError as exc:
            raise ValueError("acting port Must be a number") from exc
        if not (1 <= port <= 65535):
            raise ValueError("acting port out of range")
        if not username or not password:
            raise ValueError("acting username/password cannot be empty")
...
def choose_proxy_entry(pool: Iterable[str] | None = None, index: int | None = None) -> ProxyEntry:
    ...
    if not entries:
        raise ValueError("No proxy pool configured")
    if index is not None:
        if index < 0 or index >= len(entries):
            raise ValueError(f"Agent serial number is out of range：{index}，Available range 0-{len(entries) - 1}")
        raw = entries[index]
    else:
