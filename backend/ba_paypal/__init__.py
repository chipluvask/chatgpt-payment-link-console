# -*- coding: utf-8 -*-
"""ba_paypal — PayPal BA Authorized payment section（Restore original project progress）

Restore assets（2026-08-01）：
  - paypal/  : openai-paypal-main complete package（PayPalFlow four stages：Phase0 DataDome Over the wall
               -> Phase2 Create an account(Mail->EC) -> Phase3 Fill in the form+2FA SMS -> Phase4 authorize out BA/EUAT）
  - http_fp.py : original project `_ba_authorize_http_fp.py`（pure HTTP hCaptcha passive fingerprint stack：
               ddbm2 mint -> bridge/legacy/protocol mint -> verifyhcaptchapassive/validatecaptcha submit）
  - ba_fp_helpers/ : Node bridge（ba_hcaptcha_passive_node.js / hcaptcha_passive_node.js / ddbm2_node.js）
  - research/ : _research_semi_hybrid_mint.py（semi-hybrid green token research path，need Chrome Calculate n）

Historical stuck points（Original project progress，2026-07-16/17）：
  - DataDome ngrl hard wall  ✅ Broken（modxo_direct_signup bridge）
  - visual->passive Press the file ✅ Broken（G11 verdict=PASSIVE）
  - passive real token pure protocol mint ❌ main card point（happy-dom/pure Node PoW host_sum≡4778 constant soft-reject；
    only semi-hybrid（Chrome Calculate n）green token 2134/2143，touch"Production Zero Browser"red line）
  - green token -> EUAT Close up  ❌ Sub-stuck point（form_close back SignUpNewMember still authchallenge，EUAT Not issued）

This module provides BAAuthorizer Compatible entrance（Original min-implant ba_authorize.py external interface）：
...
# openai-paypal The module is imported with absolute（from paypal.xxx import ...），
# Bundle ba_paypal Directory added sys.path Make it directly available to backend processes import。
...
class BAAuthorizer:
    """PayPal BA protocol authorization（Original min-implant ba_authorize.py interface compatibility layer）。

    Internally driven openai-paypal-main of PayPalFlow four stages + http_fp pure HTTP passive mint。
    """
    ...
    def __init__(self, proxy: Optional[str] = None, fp_country: Optional[str] = None):
        ...
        self.session = None  # delayed until init_session / Depend on flow Created in-house
        self.csrf = ""
        self.session_id = ""
        self.ec_token = ""
        self._ba_token = ""
    ...
    def init_session(self, ba_url: str) -> dict:
        """Step 1: GET ba_url /signin，Pick ba_token / CSRF / sessionID / flowId / ec_token。

        return dict: {ba_token, page_kind, status_code, html_len, final_url, ec_token, ...}
        """
        import re as _re
        ...
        # Delayed creation PayPalSession（Reuse openai-paypal of session stack）
        ...
        # Reuse original project classification logic
    ...
    def submit_email(self, email: str) -> str:
        """Step 2: Submit email（Compatible placeholder；Complete implementation PayPalFlow._phase2_create_account）。"""
        return ""
    ...
    def authorize(
        ...
        **kwargs: Any,
        ...
        """whole BA Authorization process（drive PayPalFlow four stages）。

        sms_callback: return SMS Verification code string（Single number once）。
        """
        t0 = time.monotonic()
        ...
        if identity:
            ...
            # No context or identity: null phone Don't allow silence BR Number
            if not phone_full:
                raise ValueError(
    ...
    # ---- pure HTTP passive mint direct（original project http_fp stack）----

    def solve_captcha(self, challenge_html: str, page_url: str = "") -> dict:
        """right authchallenge HTML make pure HTTP passive mint + verify（No SMS/Do not create an account）。"""
...
def _classify_page(html: str, final_url: str = "") -> str:
    """Simple page classification（with original project _classify_ba_page Alignment）。"""
    low = (html or "").lower()
...
def _make_flow_progress_cb(on_step: Any | None):
    """Bundle flow of progress_cb(step, detail, level) Bridged as BAAuthorizer of on_step callback。

    on_step sign: on_step(idx, name, status, kw)。idx is the step number (For front-end progress bar),
    Used here uniformly 9x Segment numbers avoid being confused with authorize() Inside 0-4 rough step conflict。
    """
    _PROGRESS_IDX = {
...
class _callback_sms_provider:
    """put the old style sms_callback packaged into SmsOtpProviderProtocol。"""
