# -*- coding: utf-8 -*-
"""
sentinel_pure_vm.py — No browser Turnstile / Collector VM

Based on official sdk.js@20260219f9f6 decompiled Map-based VM：
  - Turnstile: _n(challenge, dx) → t   (bn Map)
  - Collector: jt(collector_dx, p) Side effects collection
  - Snapshot:  jt(snapshot_dx)     → so  (Reuse St state)

Algorithms and codebai.cn `_solve_turnstile_token` / realasfngl opcode table consistent：
  atob(dx) → XOR(p) → JSON command queue → opcode 0..35 Explanation and execution → btoa result

auth path with chat Paths share the same set obt Bytecode format；XOR The key is /req body of p。
"""
...
class PseudoElement:
    ...
    def toDataURL(self, *_a: Any, **_k: Any) -> str:
        # Stable synthetic canvas fingerprint（according to canvas_seed Differentiate between devices）
...
class CanvasContext:
    ...
    def getParameter(self, param: Any) -> Any:
        # Common WebGL params — vendor/renderer from matching fingerprint profile
        table = {
...
class FingerprintProfile:
    ...
    and chatgpt.choose_fp() When matching，HTTP head UA / sec-ch-ua with here navigator/screen/webgl
    Must be the same set，avoid oai-did envelope with TLS/UA contradiction（See ANTI_FUZZING.md §3.3）。
    """
    user_agent: str = (
    ...
    # and HTTP matching WebGL / canvas seed
    webgl_vendor: str = "Google Inc. (NVIDIA)"
    webgl_renderer: str = (
        "ANGLE (NVIDIA, NVIDIA GeForce GTX 1060 Direct3D11 vs_5_0 ps_5_0)"
    )
    canvas_seed: str = "default"
    # auth page common script src form
    ...
    # CDP Exported fingerprint snapshot（Optional，Override composite values）
...
def fingerprint_from_dict(
    ...
    *,
    ...
    """from chatgpt.choose_fp() of dict structure FingerprintProfile；fp=None The default random sense synthesis value is。"""
    if not fp:
    ...
    # Composite from matching fields cdp cover，and profile Fields consistent
...
class PseudoBrowser:
    ...
    def simulate_user_activity(self, duration_ms: float = 1200.0) -> Dict[str, int]:
        ...
        def _micro_pause() -> None:
            """Micro jitter between steps（ms class），break up obt Mechanical Rhythm of Collection（ANTI_FUZZING §3.1/§3.2）。"""
    ...
    def _build(self) -> None:
        ...
        nav.mimeTypes = []
        # from UA infer Chrome major，Avoid hardcoding 131 with matching fp contradiction
        _chrome_major = "131"
        try:
            import re as _re
...
def solve_auth_tso(
    ...
    *,
    ...
    fp: Optional，chatgpt.choose_fp() Returned matching fingerprint dict；and profile Choose one，
        profile priority。Maintain backward compatibility when not uploaded（Synthesize default fingerprint）。
    """
    out = PureSolveResult()
    try:
...
def build_pure_envelopes(
    ...
    *,
    ...
    fp / profile Optional：After passing in HTTP Session matching alignment（Backwards compatible with default random synthesis）。
    """
    r = solve_auth_tso(
        challenge, request_p,
