# -*- coding: utf-8 -*-
"""711 Residential Proxy Chain Relay：curl_cffi / requests → local machine relay → Clash → 711 → Target。

root cause（See PROXY_FIX.md）：
  711（global.rotgb.711proxy.com:10000）right「Direct connection to this machine」of HTTP CONNECT Cut off directly
  （Empty response / Proxy CONNECT aborted），Dantra Clash Japan's exports reconnect 711 hour CONNECT normal。
  Camoufox Can be used，It's because the system side Clash link；curl_cffi Explicitly specified 711 Shi Zou was rejected continuously。

This module starts a listening-only 127.0.0.1 of HTTP CONNECT relay：
  1) client → relay：CONNECT target:443
...
usage：
  ...
  # → "http://USER:PASS@127.0.0.1:18794"（Default port，Automatically change if occupied）
"""
...
# 711 gateway（Can be overridden by environment variables）
...
# Clash / mihomo mixed-port candidate（FlClash default 7890；history Clash Verge 7897）
CLASH_CANDIDATES = (
...
# default 18794，avoid codex Old version 18792（only CONNECT、plain text HTTP meeting 405）
...
# OpenAI/ChatGPT Support and 711 Whitelist of countries reachable by residential agents（Registered Agent Country Frame）
# It has been measured that it can be reached：US、BR（See project memory）；The rest are common residential countries and OpenAI support。
...
# ---- 711 for partial hostname CONNECT Will empty response cut off（2026-07-17 Actual measurement：
#      www.paypal.com → empty；same IP CONNECT 200 + TLS SNI normal）。
#      relay these host rewritten as IP to bypass interception。List can be expanded via environment variables。 ----
...
def _is_fake_ip(ip: str) -> bool:
    """Clash/mihomo fake-ip pool section（198.18.0.0/15）。"""
    try:
...
def _resolve_host_ip(host: str, ttl: float = 300.0) -> Optional[str]:
    """Parse the real host IP：cache first → system DNS → through Clash DoH(1.1.1.1) rollback。

    system DNS If Clash fake-ip Takeover will return 198.18.x.x（711 Can't connect），
    Identify here fake-ip After the classics Clash Walk DoH JSON get real IP and write back to cache。
    """
...
def _rewrite_connect_target(raw: str) -> str:
    """Hit rewrite list CONNECT Target host:port → ip:port（bypass 711 Hostname interception）。"""
    if ":" not in raw:
        return raw
...
def is_711_proxy(url: Optional[str]) -> bool:
    """Determine whether it is 711 residential agency URL / host:port string。"""
    if not url:
        return False
    low = url.strip().lower()
...
def parse_proxy_url(url: str) -> dict:
    """parse http://user:pass@host:port or host:port:user:pass。"""
    raw = (url or "").strip()
    if not raw:
        return {
...
def _probe_clash() -> Tuple[str, int]:
    """Detect available Clash mixed-port。"""
    global _clash_addr
    if _clash_addr:
        return _clash_addr
    for item in CLASH_CANDIDATES:
        ...
        try:
            ...
            continue
    raise RuntimeError(
        "Not found available Clash/mihomo local port（Tried 7897/17897/7890）。"
        "Please start first Clash Verge，or set environment variables CLASH_PROXY=127.0.0.1:port"
    )


def _http_connect(sock: socket.socket, hostport: str, headers: Optional[dict] = None, timeout: float = 20.0) -> bytes:
    """in the connected socket Upload HTTP CONNECT，Return response header（Contains \\r\\n\\r\\n）。"""
...
class ChainRelay:
    """local machine HTTP CONNECT relay：client → Clash → 711 → target。"""
    ...
    def start(self) -> None:
        ...
        clash = _probe_clash()
        self._clash = clash
        # Try candidate ports in sequence，If the binding is successful, the trunk will be created.（Avoid reuse and only support CONNECT old process）
        ...
        if sock is None:
            raise RuntimeError(
                f"Unable to bind 711 chain trunk port（Tried {_RELAY_PORT_CANDIDATES}）: {last_err}"
            )
        sock.listen(128)
        sock.settimeout(1.0)
        self._sock = sock
        ...
        if os.environ.get("PROXY_711_DEBUG", "0") == "1":
            print(
                f"[proxy_711] relay Started {self.listen_host}:{self.listen_port} "
    ...
    def _handle(self, client: socket.socket) -> None:
        ...
        try:
            ...
            # 1) through Clash Get through to 711 gateway
            ...
            if is_connect:
                # 2a) HTTPS：exist 711 superior CONNECT target station，Again both directions pipe
                target = _rewrite_connect_target(parts[1])  # host:port → The list is rewritten as ip:port
                resp2 = _http_connect(
                    up,
                    target,
                ...
                # 2b) plain text HTTP：Bundle absolute-form Request transfer to 711（Contains Proxy-Authorization）
                out_lines = [lines[0]]
                for ln in lines[1:]:
                    low = ln.lower()
...
def start_relay(
    ...
    """start up（or reuse）overall situation 711 chain relay（within this process daemon thread）。"""
    global _relay
    with _relay_lock:
        alive = _relay is not None and _relay._thread and _relay._thread.is_alive()
        if alive:
            # Update default credentials（session Username may be different each time）
...
def rewrite_to_relay(proxy_url: str) -> str:
    """Bundle 711 URL Rewritten to point to the local machine relay of URL（reserve user/pass）。

    enter: http://USER:PASS@global.rotgb.711proxy.com:10000
    output: http://USER:PASS@127.0.0.1:<relay_port>
    """
...
def ensure_proxy(proxy_url: Optional[str]) -> Optional[str]:
    """if 711 The proxy is rewritten as a chain relay URL；Otherwise return as is。

    No 711、None Transparent transmission。direct connection 711 exist curl_cffi Next meeting Proxy CONNECT aborted。
    """
    if not proxy_url:
        return proxy_url
    ...
    if os.environ.get("PROXY_711_DEBUG", "0") == "1":
        print(f"[proxy_711] 711 chain relay: ...@{proxy_url.split('@')[-1]} → {rewritten.split('@')[-1]}")
    return rewritten


def pick_region(exclude=None):
    """from SUPPORTED_COUNTRIES choose a country（Countries known to be unavailable can be excluded）。"""
...
def build_711_proxy(
    ...
    """Construct a 711 acting URL，And automatically rewrite to the local chain relay。"""
    import random
    import string
...
def smoke_test(proxy_url: Optional[str] = None) -> bool:
    """Quick self-test：Chain relay exit IP。"""
