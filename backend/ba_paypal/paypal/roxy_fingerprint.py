...
class RoxyApiClient:
    def __init__(self, config: RoxyCaptureConfig):
        if not config.api_key:
            raise RoxyFingerprintError("PAYPAL_ROXY_API_KEY Not configured")
        self.config = config
    ...
    def request(self, method: str, path: str, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            raise RoxyFingerprintError(f"Roxy API {path} Return non JSON response") from exc
    ...
    def create_workspace(self) -> tuple[int, str]:
        if not self.config.auto_create_workspace:
            raise RoxyFingerprintError("Roxy API no return workspace，and PAYPAL_ROXY_AUTO_CREATE_WORKSPACE=0")
        ...
        if workspace_id is None:
            raise RoxyFingerprintError(f"Roxy app API /user_add_workspace_info Not returned workspaceId: {payload}")
    ...
    def get_workspace_project(self) -> tuple[int, int | None]:
        ...
        if not rows:
            ...
            raise RoxyFingerprintError(
                "Not found Roxy workspace/team；Automatic team creation has been skipped as configured。"
                "please first Roxy Select/Create a team，or set PAYPAL_ROXY_WORKSPACE_ID。"
            )
        row = rows[0]
    ...
    def create_profile(self, workspace_id: int, project_id: int | None) -> str:
        ...
        if not dir_id:
            raise RoxyFingerprintError("Roxy /browser/create Not returned dirId")
        return dir_id
    ...
    def open_profile(self, workspace_id: int, dir_id: str) -> dict[str, Any]:
        # Roxy of Local API use `headless` Field controls headless mode。Currently opened directly by default：
        # Not first close，Not mandatory forceOpen；If you need to handle the reuse of old visible windows，via environment variables
        # PAYPAL_ROXY_CLOSE_BEFORE_OPEN / PAYPAL_ROXY_FORCE_OPEN Explicitly enabled。
        ...
        if not data.get("ws") and not data.get("http"):
            raise RoxyFingerprintError("Roxy /browser/open Not returned CDP ws/http")
        return data
    ...
    def create_or_reuse_profile(
        ...
        try:
            ...
            msg = str(exc)
            quota_markers = ("beyond", "Insufficient quota", "limit", "quota", "insufficient", "not enough")
            ...
            for p in self.list_profiles(workspace_id):
                ...
                if dir_id:
                    ...
                    return dir_id
            raise RoxyFingerprintError(
                "Roxy No existing windows to reuse，Please wait for daily limits to reset before trying again"
            )
...
def _connect_over_cdp(cdp_info: dict[str, Any]) -> str:
    ...
    if not http:
        raise RoxyFingerprintError("Roxy CDP Information is empty")
...
def _evaluate_cdp_fingerprint(
    ...
    *,
    ...
    try:
        ...
        raise RoxyFingerprintError("playwright Not installed，Unable to connect Roxy CDP") from exc
