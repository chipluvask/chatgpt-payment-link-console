# -*- coding: utf-8 -*-
"""Wallet channel wire layer: Use this project chain.py stack implementation WalletProviderTransport。

- create_checkout / stripe_init / PM / confirm / approve / poll: chain.py each stage
...
- qris charge: midtrans snap charge (payment_type=qris) + QR product
"""
...
class ChainWalletTransport:
    """Used in this project 7 segment stack + proxy pool transport。"""
    ...
    def _session(self, req: WalletTransportRequest, stage: str, spec=None,
        ...
        """chatgpt=True use chatgpt_session (bring Bearer); False use make_session (pure Stripe)."""
    ...
    def qris_charge(self, req: WalletTransportRequest) -> Mapping[str, Any]:
        """midtrans snap charge payment_type=qris (rollback gopay untokenized)。"""
        spec = _spec(req)
        ...
        try:
            ...
            # charge: First qris, Fallback on failure gopay untokenized
            attempts = (
            ...
            raise WalletProviderError(f"midtrans qris charge All failed: {last_err}",
...
def _confirm_via_chain(s, req: WalletTransportRequest, spec, *, custom: bool) -> Mapping[str, Any]:
    """Reuse stage_confirm_live of confirm body (Contains init_checksum/return_url full field)。"""
    ...
    proxy = ""  # session Already brought agent
...
def _parse_qris_charge(data: Mapping[str, Any]) -> dict[str, Any]:
    """midtrans charge response -> QR product (transplant Gpt-Agreement-Payment qris.py)。"""
