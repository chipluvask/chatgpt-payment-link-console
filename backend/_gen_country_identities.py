#!/usr/bin/env python3
"""Batch generated by country PayPal Register identity data (All true check digits of the identity field algorithm)。

usage:
    python _gen_country_identities.py US
    python _gen_country_identities.py ALL
    python _gen_country_identities.py US 10        # 10 share, default 1
...
def self_check(country: str):
    """Generate once and verify internally (for smoke test)。"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate by country PayPal Identity data")
    parser.add_argument("country", help="Country code (ISO2) or ALL")
    parser.add_argument("count", nargs="?", type=int, default=1, help="Generate copies")
    parser.add_argument("--json", default="", help="output JSON document")
    parser.add_argument("--no-verify", action="store_true", help="Skip check digit self-test")
