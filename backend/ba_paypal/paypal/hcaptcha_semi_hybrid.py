# -*- coding: utf-8 -*-
"""semi-hybrid hCaptcha passive solver (Adapt from 7/16 _research_semi_hybrid_mint.py)。

process: curl_cffi checksiteconfig -> Browser(headless) Calculate n -> happy-dom official msgpack encryption
      -> curl_cffi getcaptcha -> hsw_decrypt_resp_node.js Decrypt -> token

Basis for conclusion (docs/HCAPTCHA_PURE_PROTOCOL_ARCHIVE_20260812.md):
  - Pure protocol calculation n forever soft-reject (host_sum sentinel 4778)
  - Chrome Calculate n + Pure protocol encryption = The only one that has been verified token path (7/16: token ~2134)
"""
...
def mint_semi_hybrid(
    *,
    ...
    """Chrome Calculate n + happy-dom encryption + Pure protocol submission → token。
    ...
    # 0. preheat (curl_cffi, Same agent, take __cf_bm)
    ...
    # 1. Browser: Completed in one go V Get + checksiteconfig + Calculate n
...
def _probe_asset_v(*, sess, ua: str, proxy: str) -> str:
    """Get current asset V: priority eval Page iframe Quote, Secondly widget Page jump header, at last hsw directory detection。"""
    import re
    candidates = []
    try:
        r = sess.get(
        ...
        # eval Page dynamic loading js.hcaptcha.com api.js; widget iframe often appear in data property
        m2 = re.search(r"newassets\.hcaptcha\.com/captcha/v1/([a-f0-9]{20,})", r.text or "")
        if m2:
            candidates.append(m2.group(1))
    except Exception:
        pass
    # widget page (hcaptcha.html root path) meeting 302 arrive /captcha/v1/<V>/static/hcaptcha.html
    if not candidates:
        try:
